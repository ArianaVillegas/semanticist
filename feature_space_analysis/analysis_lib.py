...
class FeatureSpaceAnalyzer:
    ...
    def run_full_analysis(self, num_images=20, use_synthetic=False):
        ...
        if hasattr(dataset, 'targets'):
            ...
            for i, class_idx in enumerate(np.random.choice(unique_classes, size=min(num_images, len(unique_classes)), replace=False)):
                image_idx = np.random.choice(np.where(targets == class_idx)[0])
                ...
                images_to_process.append((image, class_name, image_idx))
        else: # Fallback for datasets without 'targets'
            indices = np.random.choice(len(dataset), size=num_images, replace=False)
            for i, image_idx in enumerate(indices):
                image, label = dataset[image_idx]
                images_to_process.append((image, label, image_idx))

        num_images = len(images_to_process)
        print(f"\nAnalyzing {num_images} images from distinct classes...")

        for i, (image, label, image_idx) in enumerate(images_to_process):
            print(f"Processing image {i+1}/{num_images} (Image index: {image_idx}, Class: {label})...")
            image = image.unsqueeze(0).to(self.device)
            
            results, gt_patches = self.analyze_image(image, slot_counts)
            self.create_visualizations(image_idx, label, results, gt_patches, image, slot_counts)
            
            all_results.append({'label': label, 'image_idx': image_idx, 'metrics': results})
